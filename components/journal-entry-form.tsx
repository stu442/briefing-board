  ...
  return (
    ...
    >
      ...
      <div className="mt-5 flex flex-wrap items-center justify-between gap-4 border-t border-border/60 pt-4">
        <button type="button" className="journal-photo-picker" onClick={() => photoInputRef.current?.click()}>
          <ImagePlus className="size-4" />
          <span>{photoCount ? `사진 ${photoCount}장 선택됨` : '사진 추가'}</span>
        </button>
        <input ref={photoInputRef} name="photos" type="file" accept="image/*" multiple className="sr-only" onChange={(event) => setPhotoCount(event.currentTarget.files?.length || 0)} />
        <div className="flex items-center gap-3">
