    ...
    <main className="journal-shell">
      <header className="journal-nav">
        ...
        <div className="flex items-center gap-2 sm:gap-3">
          <Link href="/journal" className="journal-nav-link">캘린더</Link>
          <Link href="/journal/write" className="journal-nav-link">글쓰기</Link>
          <Link href="/journal/retrospective" className="journal-nav-link">회고</Link>
          <JournalSyncControls />
        </div>
      </header>
        ...
        <div className="overflow-x-auto pb-2">
          ...
          {calendarDays.map((day, index) => {
            ...
            return <Link key={slug} href={`/journal/${slug}`} className={`relative aspect-square overflow-hidden rounded-xl border p-1.5 text-left transition hover:-translate-y-0.5 hover:border-[hsl(var(--journal-accent))] sm:p-2 ${isToday ? 'border-[hsl(var(--journal-accent))]' : 'border-border/60'}`} style={note?.coverImage ? { backgroundImage: `linear-gradient(180deg, rgba(13, 16, 24, .05), rgba(13, 16, 24, .82)), url(${note.coverImage})`, backgroundSize: 'cover', backgroundPosition: 'center' } : undefined}>
              ...
              {note ? <span className={`relative z-10 mt-1 block line-clamp-2 text-[10px] leading-3 sm:line-clamp-3 ${note.coverImage ? 'text-white/90' : 'text-muted-foreground'}`}>{note.preview}</span> : <span className="relative z-10 mt-1 block text-[10px] text-muted-foreground/60">기록하기</span>}
      ...
      <details className="journal-timeline rounded-3xl" open={false}>
        <summary className="flex cursor-pointer list-none items-end justify-between gap-4"><div><p className="journal-section-label">YOUR DAYS</p><h2>지난 기록</h2></div><span className="text-xs text-muted-foreground">{recentNotes.length}개 · 펼치기</span></summary>
        <div className="mt-5">
